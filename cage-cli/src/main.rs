...
use crate::docker::parse::{Directive, DockerfileDecoder, Mode, DecodeError};
...
async fn main() {
    ...
    let built_dockerfile = match process_dockerfile(dockerfile).await {
        ...
        Err(e) => {
            eprintln!("An error occurred while processing your dockerfile - {:?}", e);
            return;
        }
    };
    ...
    let _temp_output_dir = enclave_builder::build_enclave(
...
async fn process_dockerfile<R: AsyncRead + std::marker::Unpin>(dockerfile_src: R) -> Result<Vec<Directive>, DecodeError> {
    ...
    let cleaned_instructions: Vec<Directive> = instruction_set.into_iter()
        .filter(remove_unwanted_directives)
        .collect();

    let (user_service_builder, user_service_location) = docker::utils::create_combined_docker_entrypoint(last_entrypoint, last_cmd).map(|entrypoint| {
        ...
        (Directive::new_run(user_service_builder_script), user_service_runner)
    })?;
    ...
    let injected_directives = vec![
        ...
        Directive::new_entrypoint(Mode::Exec, vec!["runsvdir".to_string(), "/etc/service".to_string()])
    ];
...
"#;
        ...
        for (expected_directive, processed_directive) in zip(expected_directives.iter(), processed_file.iter()) {
            assert_eq!(expected_directive.to_string(), processed_directive.to_string());
        }
    }
}
