    ...
    loop {
        ...
        if let Ok((mut connection, _client_socket_addr)) = tcp_listener.accept().await {
            ...
            tokio::spawn(async move {
                log::debug!("Accepted incoming TCP stream — {_client_socket_addr:?}");
                let enclave_stream =
                ...
                if should_remove_proxy_protocol(data_plane_version.as_ref()) {
                    ...
                    log::warn!("An error occurred while piping the connection over vsock - {e:?}");
                }
            });
        }
    }
...
fn listen_for_shutdown_signal() {
    log::info!("Setting up listener for SIGTERM");
    tokio::spawn(async {
        let sns_client =
        ...
        let _ = ctrlc::set_handler(move || {
            tx.send(()).unwrap_or_else(|err| {
                log::error!("Could not broadcast sigterm to channel: {err:?}");
            })
        })
        .map_err(|err| {
        ...
        match rx.recv().await {
            Some(_) => {
                log::info!("Received SIGTERM - sending message to SNS");
